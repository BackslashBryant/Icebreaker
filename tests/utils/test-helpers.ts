...
export async function waitForBootSequence(
  ...
  // Start timing at the beginning of boot sequence
  if (telemetry) {
  ...
  try {
    ...
    // End timing after boot sequence completes
    if (telemetry) {
...
export async function completeOnboarding(
  ...
  try {
    ...
    // End onboarding timing after successful navigation to Radar
    if (telemetry) {
    ...
    // Ensure onboarding timing is recorded even if onboarding fails
    if (telemetry) {
      telemetry.endTiming('onboarding');
